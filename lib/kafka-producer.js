...
Producer.prototype.getProducer = Promise.method(function (opts, topts) {
  ...
  this.log.info('getProducer() :: Starting producer with options:', opts);
  ...
  return new Promise((resolve, reject) => {
    producer.on('ready', () => {
      this.log.debug('getProducer() :: Got "ready" event.');
      resolve(producer);
    });

    producer.connect({}, (err) => {
      if (err) {
        this.log.error('getProducer() :: Connect failed:', err);
        reject(err);
        return;
      }
      this.log.debug('getProducer() :: Got "connect()" callback.');
...
Producer.prototype._serializeType = function (topicName, isKey, data) {
  ...
  if (!schema) {
    this.log.warn('_produceWrapper() :: Warning, did not find topic on SR for ' + schemaType + ' schema:',
      topicName);
    ...
    this.log.debug(`found topic ${topicName} for procuder with id ${schemaId} and is key: ${isKey}`);
